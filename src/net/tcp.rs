...
use core::panic;
use liburing_rs::{
    IORING_ASYNC_CANCEL_ALL, IORING_RECVSEND_BUNDLE, IOSQE_BUFFER_SELECT, IOSQE_CQE_SKIP_SUCCESS,
    IOSQE_FIXED_FILE, io_uring_prep_cancel64, io_uring_prep_close_direct,
...
impl TcpListener {
    ...
    pub fn cancel(&self) -> impl Future<Output = Result<()>> {
        ...
        let key = listener
            ...
            .io_ops
            .borrow_mut()
            .insert(make_io_uring_op(ref_count, OpType::FdCancel), &listener.fd_impl.ex);
    ...
    pub fn close(&self) -> impl Future<Output = Result<()>> {
        ...
        let key = acceptor_impl
            ...
            .io_ops
            .borrow_mut()
            .insert(make_io_uring_op(ref_count, OpType::FdClose), &acceptor_impl.fd_impl.ex);
...
impl Drop for TcpListener {
    fn drop(&mut self) {
        ...
        if unsafe { (*rc).obj_count } == 1 {
            let acceptor_impl = unsafe { &mut *self.p.as_ptr() };
            if acceptor_impl.accept_op.is_some() {
                let key_data = acceptor_impl.accept_op.take().unwrap();
...
impl Future for AcceptFuture<'_> {
    ...
    fn poll(
        ...
        let ex = acceptor_impl.fd_impl.ex.clone();
        ...
        match acceptor_impl.accept_op {
            None => {
                ...
                let fd = acceptor_impl.fd_impl.fd;
                ...
                acceptor_impl.accept_op = Some(user_data);
...
impl TcpStream {
    ...
    pub fn shutdown(&self, how: i32) -> impl Future<Output = Result<()>> {
        ...
        let key = stream_impl
            ...
            .io_ops
            .borrow_mut()
            .insert(make_io_uring_op(ref_count, OpType::TcpShutdown), &stream_impl.fd_impl.ex);
    ...
    pub fn close(&self) -> impl Future<Output = Result<()>> {
        ...
        let key = stream_impl
            ...
            .io_ops
            .borrow_mut()
            .insert(make_io_uring_op(ref_count, OpType::FdClose), &stream_impl.fd_impl.ex);
    ...
    pub fn cancel(&self) -> impl Future<Output = Result<()>> {
        ...
        let key = stream_impl
            ...
            .io_ops
            .borrow_mut()
            .insert(make_io_uring_op(ref_count, OpType::FdCancel), &stream_impl.fd_impl.ex);
        ...
        if unsafe { (*rc).obj_count } == 1 {
            ...
            if stream_impl.recv_op.is_some() {
                ...
                if op.done {
                    ...
                    let user_data = key_data;
                    // Makes sure this gets cleaned up, not really an
                    // eager-dropped operation.
                    op.eager_dropped = true;
